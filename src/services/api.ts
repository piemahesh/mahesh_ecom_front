...
export const productsAPI = {
  getProducts: async (params?: { page?: number; search?: string; category?: string }) => {
  ...
  updateProduct: async (id: number, productData: FormData) => {
    const response: AxiosResponse = await api.put(`/products/${id}/`, productData, {
      headers: {
        'Content-Type': 'multipart/form-data',
      },
    });
...
export const ordersAPI = {
  ...
  createOrder: async (orderData: OrderCreate): Promise<Order> => {
    const response: AxiosResponse = await api.post('/orders/create/', orderData);
    return response.data;
  },
  ...
  cancelOrder: async (orderId: string) => {
    const response: AxiosResponse = await api.post(`/orders/${orderId}/cancel/`);
    return response.data;
  },
  
  downloadReceipt: async (orderId: string) => {
    const response: AxiosResponse = await api.get(`/orders/${orderId}/receipt-pdf/`, {
      responseType: 'blob',
    });
    return response.data;
  },
